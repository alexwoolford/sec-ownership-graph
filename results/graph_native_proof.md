...
- **Recursive BFS:** `AAPL`→`JPM` 1 hops (37.89 ms); `KO`→`BA` 2 hops (22.92 ms); `NVDA`→`WMT` 2 hops (14.8 ms). Same paths as the graph, at higher cost — frontier expansion rather than bidirectional search, and it needs an explicit hop cap to stay bounded.
...
- **Recursive CTE:** largest component 25 members in 2.15 ms — **agrees with the graph**.
