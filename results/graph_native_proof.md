...
*Generated 2026-08-04 read-only against `secgraph`. Each win runs the graph
...
- **Recursive CTE (`WITH RECURSIVE`):** 30 paths at >=2 hops, deepest 3, in 0.75 ms — **agrees with the graph**. A warehouse can reach this answer; the graph's advantage is one declarative pattern per question rather than a hand-built recursion each time.
...
- **Recursive BFS:** `AAPL`→`JPM` 1 hops (37.01 ms); `KO`→`BA` 2 hops (22.68 ms); `NVDA`→`WMT` 2 hops (14.2 ms). Same paths as the graph, at higher cost — frontier expansion rather than bidirectional search, and it needs an explicit hop cap to stay bounded.
...
- **Recursive CTE:** largest component 25 members in 2.07 ms — **agrees with the graph**.
